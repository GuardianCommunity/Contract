...
contract ArraySupply
{
    ...
    Supply[] private Storage;

    function IsAvailable(address token) internal view returns (uint256 Index, bool Result)
    {
    ...
    function Increase(address token, uint256 amount) external
    {
    ...
    function Balance(address token) external view returns (uint256 Value, bool Result)
    {
        for (uint256 I = 0; I < Storage.length; I++)
        {
            if (Storage[I].Token == token)
    ...
    function Balance() external view returns (address[] memory Token, uint256[] memory Value)
    {
        for (uint256 I = 0; I < Storage.length; I++)
        {
